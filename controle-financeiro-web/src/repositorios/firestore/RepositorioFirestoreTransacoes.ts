...
import type { IRepositorioTransacoes } from '../interfaces';

/// Implementação Firestore de `usuarios/{uid}/transacoes`.
///
...
function colecaoDeTransacoes(uid: string) {
...
export class RepositorioFirestoreTransacoes implements IRepositorioTransacoes {
  observar(
    uid: string,
    inicio: Date,
    fim: Date,
  ...
  async ler(uid: string, inicio: Date, fim: Date): Promise<Transacao[]> {
    const resultado = await getDocs(consultaDoPeriodo(uid, inicio, fim));
    return resultado.docs.map(paraTransacao);
  }

  async criar(uid: string, dados: DadosDeTransacao): Promise<void> {
  ...
  async atualizar(uid: string, id: string, dados: DadosDeTransacao): Promise<void> {
    const referencia = doc(bancoDeDados, 'usuarios', uid, 'transacoes', id);
...
export const repositorioTransacoesFirestore = new RepositorioFirestoreTransacoes();
